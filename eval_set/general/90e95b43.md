---
id: 90e95b43
title: Normalized situationalawareness.pdf
source_url: local://C:\Users\kenhu\llm-wiki\raw\manual\situationalawareness.pdf
source_type: doc
domain: general
ingested_at: '2026-04-05T19:44:47.274140+00:00'
confidence: 0.8
verified: false
---
...
hitting a wall.1 But by counting the OOMs, we get a peek at 1 Predictions they’ve made every year
for the last decade, and which they’ve
been consistently wrong about. . .what we should actually expect.
...
tences from the article. 3 3 From the GPT-2 paper, Section 3.6.
...
might know from ChatGPT.shot examples it can do some simple useful tasks.” It started
...
imal progress over the coming years (Figure 8);7 and yet within 7 A coauthor notes: “When our group
...
proven wrong.8 If there’s one lesson we’ve learned from the past 8 Here’s Yann LeCun predicting in 2022
...
pute, in the example of OpenAI Sora.With each OOM of effective compute, models predictably, reliably get
...
3.75-level” and GPT-4-level performance,15 while costing 15 Gemini 1.5 Flash ranks similarly to
...
scale.Put together, public information suggests that the GPT- 2 to
...
of billions or more, given cluster costs.at least, don’t seem to have slowed down at all.) On the high
end, we could even see more fundamental, Transformer-like 21 21 Very roughly something like a ~ 10x
gain.breakthroughs with even bigger gains.
...
possible—is useful here as well. 24 24 See also Andrej Karpathy’s talk
...
els have incredible latent capabilities,25 but they’re raw and 25 That’s the magic of unsupervised
...
to write an essay via their initial stream-of-consciousness. 27 27 People are working on this though.
...
problem or project on its own. 28 28 Which makes sense—why would
...
II outer loop 29 that lets it reason through difficult, long- 29 System I vs. System II is a useful
...
gap we’ll cover once more, where will that take us?31 We 31 Of course, any benchmark we have
...
bygone heyday, Moore’s law was only1–1.5 OOMs/decade.
...
cision number formats, from fp 64/fp32 for traditional
...
country-annihilating device.32 32 And much of the Cold War’s per-
...
and their labs’ competitive edge.It’s worth emphasizing just how straightforward and hacky
...
~$1 hour per GPU if A 100-equivalents.
...
the automated researchers.38) 38 Jacob Steinhardt estimates that kˆ3
parallel copies of a model can be
replaced with a single model that is kˆ 2
...
tomated Alec Radfords.40 I think just about every researcher at 40 Alec Radford is an incredibly gifted
...
limits,41 if we got 5 OOMs in the last decade, we should 41 25 OOMs of algorithmic progress on
...
~10 FLOPs.43) But something like 5 OOMs seems very 43 Though you could get results that
would take 25 more OOMs of hardware
with current architecture!much in the realm of possibilities; again, that would just re-
...
sustaining progress.44 44 100x progress → 100x more auto-
...
sizeable (many OOMs) one-time gain. 45 45 Analogously, in semi-endogenous
...
speed!)46 would make extraordinary advances in many fields 46 The 10x speed robots doing physical
...
covering all of the Nevada desert 47). The increase in growth 47 Why isn’t “factorio-world”—build
...
Training compute
Earlier, we found a roughly ~ 0.5 OOMs48/year trend growth of 48 As mentioned earlier, OOM = or-
der of magnitude, 10x = 1 order of
magnitudeAI training compute. If this trend were to continue for the rest
...
and datacenter construction. 49 While it may take you a year 49 One key uncertainty is how dis-
...
large fraction of GPUs will probably be be used for inference 50 50 See, for example, Zuck here; only
~45k of his H 100s are in his largest
...
than the chips (site, building, cooling, power, etc.). 51 51 For example, this total-cost-of-
...
training clusters, let’s say 2x/year.53 53 A larger fraction of global GPU
...
many-hundred-billion dollar corporate bond sale then. 55 55 What will happen to interest rates
...
cluster by the end of the decade seems on the table. 56 56 And, farther out, but if AGI truly led
...
abundant natural gas. 58 58 Thanks to Austin Vernon (private
...
ter.59 Each rig can drill roughly 3 wells per month, so 40 59 New wells produce around 0.01 BCF
per day.rigs (the current rig count in the Marcellus) could build
...
power multiple trillion-dollar datacenters.62 62 A cubic foot of natural gas generates
...
about $100B of capex 63 for 100GW of natural gas power 63 The capex costs for natural gas power
...
AI. TSMC ~doubled 66 in the past 5 years; they’d likely need to 66 (Using revenue as a proxy.)
...
risks the AGI weights getting stolen 67 (and perhaps be shipped 67 It’s a lot easier to do side-channel at-
tacks to exfiltrate weights with physical
access!to China); it risks these dictatorships physically seizing the dat-
...
whiteboard in March of 2023.capital acceleration has been put into motion.
...
growth in Nvidia revenue from CY 24 to
...
most capable state actors). 72 They admit to being at level 0 72 Based off of their claimed correspon-
dence of their security levels to RAND’s
weight security report’s L1-L5(only the most banal and basic measures). If we got AGI and
...
instantly leak the algorithmic secrets. 73 73 I sometimes joke that AI lab algo-
...
any point.74 You can . . . just look through office windows. And 74 Indeed, I've heard from friends
...
the same intense security! 75 75 Inference fleets will likely be much
...
the intelligence explosion and post-superintelligence. 77 A 77 E.g., space to take an extra 6 months
during the intelligence explosion
...
dangerous, reckless, and unpredictable actors. 78 78 We try really hard to prevent nuclear
...
in fission matters.”79 But he was rebuffed by most; secrecy was 79 The Making of the Atomic Bomb, p. 509
...
that ultimately doomed the German nuclear weapons effort. 80 80 The Making of the Atomic Bomb, p. 507
...
be worth hundreds of billions of dollars to the CCP .81 The real- 81 100x+ compute efficiencies, when
clusters worth $10s or $100s of billions
are being built.ity is that, a) in the next 12-24 months, we will develop the key
...
plan for how we’ll muddle through,84 and why I’m optimistic. 84 As Tyler Cowen says, muddling
through is underrated!While not enough people are on the ball—we should have
...
scale to superintelligence.Even now, AI labs already need to pay expert software engi-
...
~$100/hour for GPQA questions 87 in the last few years. In the 87 “We estimate an average hourly
payment of approximately $95 per
hour,” p.3 of the GPQA paper.(near) future, even the best human experts spending lots of
...
ness and make money 88 (as a simplified example): 88 Very simplified, think of an AI system
...
in models with only unsupervised data. 92 ROME is able to 92 Essentially, needing only the con-
...
exfiltration94 across the board, from hardware encryption to 94 A model stealing its own weights,
to make copies of itself outside of the
original datacenter.many-key signoff.95 And once our AGIs can do automated
...
picking up some abstruse malign behavior. 96 Moreover, while 96 Though, of course, while that may
...
to align “somewhat-superhuman” systems will mostly work. 97 97 To be clear, given the stakes, I think
...
plan is you and your 20 colleagues (much more stressful!). 98 98 I’m reminded of Scott Aaronson’s
...
the Yi-34B architecture seems to have essentially the Llama 2
architecture, with merely a few lines of code changed 100). Chi- 100 Similarly, Qwen hugging face code
...
training clusters in a few years. 106 106 Note that even 3x more on chips
...
capabilities.107 107 Some argue that even if China stole
...
that’swhen we offer a deal to China and other adversaries.
...
the Cold War in one piece involved too much luck 111—and the 111 Daniel Ellsberg recounts this riv-
...
incentives for first-strikes will be tremendous. 112 112 Everyone will be racing to their own
...
whatever (though it might!).113 Rather, I expect a more suave 113 Note that while private companies
...
trillions of investment involved, and for checks-and-balances. 114 114 Congress—even the Vice President!—
...
to literally coup the US government .115 Imagine if Elon Musk 115 It wouldn’t even require cooper-
...
the only remedy, I’ve never stopped since then. It’s28 years,
...
cost.
• Alternatively, you can estimate it as $ 25k cost per H 100, 10k
...
around 40% of a large cluster cost is the H 100 GPUs itself,
...
but not a ton. E.g., the H 100 -> B100 is likely something like
1.5x FLOP/$: B 100s are effectively two H100s stapled together,
but retailing for < 2x the cost. But the B 100 was somewhat
of an exception. They are surprisingly cheap, likely because
Nvidia wants to crush competition. By contrast, A 100s ->
H100s weren’t much of a FLOP/$ improvement (2x better chip
without fp8, roughly 2x the cost), maybe 1.5x if we count fp 8
...
AI (e.g. specialized for Transformers, and already at fp 8/fp4
...
working, storage, and so on (which in the H 100 numbers above
